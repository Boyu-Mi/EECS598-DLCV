...
def content_loss(content_weight, content_current, content_original):
    ...
    F = content_current.squeeze().flatten(start_dim=1)
    P = content_original.squeeze().flatten(start_dim=1)
...
def gram_matrix(features, normalize=True):
    ...
    features_flatten = features.flatten(start_dim=2)
    features_flatten_ = features_flatten.transpose(1, 2)
    gram = features_flatten.bmm(features_flatten_)
    if normalize:
        gram /= (H * W * C)
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, idx in enumerate(style_layers):
        g = gram_matrix(feats[idx].clone())
        g_ = style_targets[i]
        loss += style_weights[i] * ((g - g_) ** 2).sum()
        
    return torch.tensor([loss]).to(feats[0].device)
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def tv_loss(img, tv_weight):
    ...
    # Replace "pass" statement with your code
    _, _, H, W = img.shape
    term1 = ((img[:, :, 1:, :] - img[:, :, :-1, :]) ** 2).sum()
    term2 = ((img[:, :, :, 1:] - img[:, :, :, :-1]) ** 2).sum()
    return tv_weight * (term1 + term2)
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################

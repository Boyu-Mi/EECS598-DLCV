...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.encoder = nn.Sequential(
            nn.Flatten(), nn.Linear(input_size, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim),
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_size, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim), 
             nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, input_size),
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.hidden_dim = 256
        self.encoder = nn.Sequential(
            nn.Flatten(), nn.Linear(input_size + self.num_classes, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim), nn.ReLU()
        )
        self.mu_layer = nn.Linear(self.hidden_dim, self.latent_size)
        self.logvar_layer = nn.Linear(self.hidden_dim, self.latent_size)
        ...
        # Replace "pass" statement with your code
        self.decoder = nn.Sequential(
            nn.Linear(latent_size + self.num_classes, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim), 
             nn.ReLU(), nn.Linear(self.hidden_dim, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, input_size),
             nn.Sigmoid(), nn.Unflatten(1, (1, 28, 28)) 
        )
        ###########################################################################
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        x = torch.hstack((x.flatten(start_dim=1), c))
        latent = self.encoder(x)
        mu = self.mu_layer(latent)
        logvar = self.logvar_layer(latent)
        z = reparametrize(mu, logvar)
        x_hat = self.decoder(torch.hstack((z, c)))
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
        return x_hat, mu, logvar
...
def reparametrize(mu, logvar):
    ...
    eps = torch.normal(torch.zeros_like(mu), torch.ones_like(logvar)).to(mu.device)
    z = eps * sigma + mu
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
...
def loss_function(x_hat, x, mu, logvar):
    ...
    sigma_2 = torch.exp(logvar)
    loss2 = torch.sum(1 + logvar - mu ** 2 - sigma_2) * -0.5
    loss = (loss1 + loss2)/ mu.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
...
def compute_saliency_maps(X, y, model):
    ...
    res = model(X) #(N, C)
    scores = torch.gather(res, dim=1, index=y.view(-1, 1)).squeeze()
    loss = scores.sum()
    loss.backward()
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        target = scores[0, target_y].squeeze()
        if verbose:
            print('Iteration %d: target score %.3f, max score %.3f' % (i, target.item(), max_score))
        if cls.item() == target_y:
            break
        target.backward()
        X_adv.data += learning_rate * X_adv.grad.data / torch.linalg.norm(X_adv.grad)
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return X_adv
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    scores = model(img)
    target = scores[0, target_y].squeeze()
    loss = target - l2_reg * torch.linalg.norm(img) ** 2
    loss.backward()

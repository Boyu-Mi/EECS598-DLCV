...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            ...
            for c in token.__str__():
                out.append(token_dict[str(c)])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        query_ = query[i] #(M, )
        scalars = torch.zeros_like(query[:, 0])
        for j in range(K):
            key_ = key[j] #(M, )
            s = query_.dot(key_).item() / math.sqrt(M)
            scalars[j] = s
        
        scalars = torch.softmax(scalars, dim=0) #(K, )
        out[i] = value.T.mv(scalars)
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(K):
        query_ = query[:, i, :] #(N, M)
        scalars = torch.zeros_like(query[:, :, 0]) #(N, K)
        for j in range(K):
            key_ = key[:, j, : ] #(N, M)
            s = query_.unsqueeze(1).bmm(key_.unsqueeze(-1)).squeeze() / math.sqrt(M) # (N, )
            scalars[:, j] = s
        scalars = torch.softmax(scalars, dim=1) #(N, K)
        out[:, i, :] = value.permute(0, 2, 1).bmm(scalars.unsqueeze(-1)).squeeze(-1)
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights = query.bmm(key.permute(0, 2, 1)) / M ** 0.5 #(N, K, K)
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # weights.masked_fill(mask, -1e9)
        weights = weights.masked_fill(mask, -1e9)
        # print("weight.shape ", weights.shape)
        # print("mask.shape ", mask.shape) 
        
    weights_softmax = torch.softmax(weights, dim=2) # why dim=2? :(
    # Replace "pass" statement with your code
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        torch.nn.init.uniform_(self.v.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        args:
            query: Tensor of shape (N, K, M) (N, K, dim_in)
            key: Tensor of shape (N, K, M) (N, K, dim_in)
            value: Tensor of shape (N, K, M) (N, K, dim_in)
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        key_vectors = self.k(key) # (N, K, dim_q)
        value_vectors = self.v(value) #(N, K, dim_v)
        query_vectors = self.q(query) #(N, K, dim_q)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(query_vectors, key_vectors, value_vectors, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # each layer: (N, K, dim_in) * 3 => (N, K, dim_out)        
        self.out_trans_layer = nn.Linear(num_heads * dim_out, dim_in)
        c = math.sqrt(6 / (dim_in + dim_out))
        torch.nn.init.uniform_(self.out_trans_layer.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for layer in self.heads:
            y_lst.append(
                layer(query, key, value, mask)
            )
        #  y_lst (num_head, N, K, dim_out)
        y = torch.cat(y_lst, dim=2)
        y = self.out_trans_layer(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.gamma = nn.parameter.Parameter(torch.ones(emb_dim))
        self.beta = nn.parameter.Parameter(torch.zeros(emb_dim))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x: Tensor):
        ...
        if len(x.shape) == 3:
            N, K, M = x.shape
            mean = x.mean(dim=2).unsqueeze(-1).repeat(1, 1, M) #(N, K, M) 
            std = (torch.sum((x - mean)**2, dim=2, keepdim=True) / M) ** 0.5
        else:
            N, K = x.shape
            mean = x.mean(dim=1).unsqueeze(-1).repeat(1, K)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.first_layer = nn.Linear(inp_dim, hidden_dim_feedforward)
        c = math.sqrt(6 / (inp_dim + hidden_dim_feedforward))
        torch.nn.init.uniform_(self.first_layer.weight, -c, c)
        self.second_layer = nn.Linear(hidden_dim_feedforward, inp_dim)
        c = math.sqrt(6 / (inp_dim + hidden_dim_feedforward))
        torch.nn.init.uniform_(self.second_layer.weight, -c, c)
        self.relu = nn.ReLU()
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multihead = MultiHeadAttention(num_heads, emb_dim, dim_out=emb_dim // num_heads)
        self.layernorm1 = LayerNormalization(emb_dim)
        self.layernorm2 = LayerNormalization(emb_dim)
        self.feedback = FeedForwardBlock(inp_dim=emb_dim, hidden_dim_feedforward=feedforward_dim)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    # why is it so slow? almost same time as loop :(
    tmp = torch.ones((K, K), device=seq.device, dtype=torch.bool)
    tmp[tmp.triu(diagonal=1) > 0] = True
    tmp[tmp.triu(diagonal=1) == 0] = False
    
    mask = tmp.repeat(N, 1, 1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        # layer_norm(out3 + out2) - dropout - out4 - feed_forward - out5 - \
        # layer_norm(out5 + out4) - dropout - out
        # mask = get_subsequent_mask(dec_inp)
        out1 = self.attention_self.forward(dec_inp, dec_inp, dec_inp, mask)
        out2 = self.dropout(self.norm1(out1 + dec_inp))
        out3 = self.attention_cross.forward(query=out2, key=enc_inp, value=enc_inp)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.arange(0, 1, 1 / K).unsqueeze(0).unsqueeze(-1).repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        decoder_out = self.decoder(a_emb_inp, encoder_out, mask)

...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        std1[:] = weight_scale
        self.params['W1'] = torch.normal(mean=0., std=std1).to(device).to(dtype)
        self.params['b1'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        std2 = torch.zeros((hidden_dim, num_classes))
        std2[:] = weight_scale
        self.params['W2'] = torch.normal(mean=0., std=std2).to(device).to(dtype)
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * torch.sum(W1 ** 2) + self.reg * torch.sum(W2 ** 2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            output_size = num_classes if i == self.num_layers - 1 else hidden_dims[i]
            self.params[weight_name] = torch.normal(mean=0, std=weight_scale, size=(input_size, output_size), device=device, dtype=dtype)
            self.params[bias_name] = torch.zeros(output_size, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            hidden_out, cache = Linear_ReLU.forward(hidden_out, weight, bias)
            hidden_cache.append(cache)
            if self.use_dropout:
              hidden_out, dropcache = Dropout.forward(hidden_out, self.dropout_param)
              dropout_cache.append(dropcache)
        i = self.num_layers - 1
        ord = str(i + 1)
        weight_name = 'W' + ord
        bias_name = 'b' + ord
        weight = self.params[weight_name]
        bias = self.params[bias_name]
        scores, cache = Linear.forward(hidden_out, weight, bias)
        hidden_cache.append(cache)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        #####################################################################
        # Replace "pass" statement with your code
        # hidden_cache = [X, h1, h2]
        b = X.shape[0]
        loss, upstream_grad = softmax_loss(scores, y) # (batch, num_classes)
        i = self.num_layers - 1
        ord = str(i + 1)
        weight_name = 'W' + ord
        bias_name = 'b' + ord
        weight = self.params[weight_name]
        bias = self.params[bias_name]
        upstream_grad, dw, db = Linear.backward(upstream_grad, hidden_cache[-1])
        grads[weight_name] = dw + 2 * self.reg * weight
        grads[bias_name] = db
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            weight_name = 'W' + ord
            bias_name = 'b' + ord
            weight = self.params[weight_name] #(hidden_dim1, hidden_dim2)
            bias = self.params[bias_name]
            if self.use_dropout:
                upstream_grad = Dropout.backward(upstream_grad, dropout_cache[i])
            upstream_grad, dw, db = Linear_ReLU.backward(upstream_grad, hidden_cache[i])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, optim_config={'learning_rate': 0.1}, lr_decay=0.95, num_epochs=10, 
                    batch_size=64, print_every=100, device=device)
    solver = Solver(model, 
    data_dict, 
    update_rule = sgd,
    optim_config = {
...
def rmsprop(w, dw, config=None):
    ...
    new_cache = dr * cache+ (1 - dr) * dw ** 2
    next_w = w - config['learning_rate'] * dw / (torch.sqrt(new_cache) + config['epsilon'])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################

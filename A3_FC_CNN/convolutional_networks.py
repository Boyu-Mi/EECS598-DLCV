...
class MaxPool(object):
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros_like(x).to(device = x.device, dtype = x.dtype)
        ...
        for i in range(h_):
            for j in range(w_):
                ...
                j_x = j * stride
                slice_ = x[:, :, i_x: i_x + pool_height, j_x: j_x + pool_width]  # shape (N, C, hh, ww)
                max_mask = torch.amax(slice_, dim = (2, 3))
                mask = (slice_ == max_mask.unsqueeze(-1).unsqueeze(-1))
                dx[:, :, i_x: i_x + pool_height, j_x: j_x + pool_width] += \
                    mask * (dout[:, :, i, j]).unsqueeze(-1).unsqueeze(-1)
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, num_filter in enumerate(num_filters):
            in_channel = C if i == 0 else num_filters[i - 1]
            out_channel = num_filter
            ord = str(i + 1)
            weight_name, bias_name = 'W' + ord, 'b' + ord
            if self.batchnorm:
                self.params['gamma' + ord] = 0.01 * torch.randn(num_filter, device=device, dtype=dtype)
                self.params['beta' + ord] = 0.01 * torch.randn(num_filter, device=device, dtype=dtype)
            # print(out_channel, in_channel)
            if weight_scale == 'kaiming':
                self.params[weight_name] = kaiming_initializer(in_channel, out_channel, 3, relu=True, device=device, dtype=dtype)
            else:
                self.params[weight_name] = weight_scale * torch.randn(out_channel, in_channel, 3, 3, dtype=dtype, device=device)
            self.params[bias_name] = torch.zeros(out_channel, dtype=dtype, device=device)
            if i in max_pools:
                H = (H - 2) // 2 + 1
                W = (W - 2) // 2 + 1
        ...
        weight_name, bias_name = 'W' + ord, 'b' + ord
        if weight_scale == 'kaiming':
            self.params[weight_name] = kaiming_initializer(H * W * out_channel, num_classes, None, relu=False, device=device, dtype=dtype)
        else:
            self.params[weight_name] = weight_scale * torch.randn(H * W * out_channel, num_classes, dtype=dtype, device=device)
        self.params[bias_name] = torch.zeros(num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ord = str(i + 1)
            weight = self.params['W' + ord]
            bias = self.params['b' + ord]
            if self.batchnorm:
                gamma, beta = self.params['gamma' + ord], self.params['beta' + ord]
                if i in self.max_pools:
                    out, cache = Conv_BatchNorm_ReLU_Pool.forward(out, weight, bias, gamma, beta, conv_param, 
                                                                  self.bn_params[i], pool_param)
                else:
                    out, cache = Conv_BatchNorm_ReLU.forward(out, weight, bias, gamma, beta, conv_param, 
                                                                  self.bn_params[i])
            else:
                if i in self.max_pools:
                    out, cache = Conv_ReLU_Pool.forward(out, weight, bias, conv_param, pool_param)
                else:
                    out, cache = Conv_ReLU.forward(out, weight, bias, conv_param)
            caches.append(cache)
        i += 1
        weight = self.params['W' + str(i + 1)]
        bias = self.params['b' + str(i + 1)]
        scores, cache = Linear.forward(out, weight, bias)
        caches.append(cache)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        i = str(self.num_layers)
        upstream_grad, dw, db = Linear.backward(upstream_grad, caches[self.num_layers - 1])
        grads['W' + i] = dw + 2 * self.reg * self.params['W' + i]
        grads['b' + i] = db
        loss += self.reg * torch.sum(self.params['W' + i] ** 2)
        assert len(caches) == self.num_layers
        for i in range(self.num_layers - 2, -1, -1):
            weight_name = 'W' + str(i + 1)
            bias_name = 'b' + str(i + 1)
            loss += self.reg * torch.sum(self.params[weight_name] ** 2)
            if self.batchnorm:
                if i in self.max_pools:
                    upstream_grad, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(upstream_grad, caches[i])
                    grads[weight_name] = dw + 2 * self.reg * self.params[weight_name]
                    grads[bias_name] = db
                    grads['gamma' + str(i + 1)] = dgamma
                    grads['beta' + str(i + 1)] = dbeta
                else:
                    upstream_grad, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(upstream_grad, caches[i])
                    grads[weight_name] = dw + 2 * self.reg * self.params[weight_name]
                    grads[bias_name] = db
                    grads['gamma' + str(i + 1)] = dgamma
                    grads['beta' + str(i + 1)] = dbeta
                ...
                if i in self.max_pools:
                    upstream_grad, dw, db = Conv_ReLU_Pool.backward(upstream_grad, caches[i])
                    grads[weight_name] = dw + 2 * self.reg * self.params[weight_name]
                    grads[bias_name] = db
                else:
                    upstream_grad, dw, db = Conv_ReLU.backward(upstream_grad, caches[i])
                    grads[weight_name] = dw + 2 * self.reg * self.params[weight_name]
                    grads[bias_name] = db
        #############################################################
        #                       END OF YOUR CODE                    #
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        scale = (gain / Din) ** 0.5
        weight = scale * torch.randn(Din, Dout, dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        scale = (gain / (Din * K * K)) ** 0.5
        weight = scale * torch.randn(Dout, Din, K, K, dtype=dtype, device=device)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        #https://www.jianshu.com/p/04bc8026d3c4
        dx_hat = dout * gamma
        dvar = torch.sum(dx_hat * (x - mean) * (-0.5) * (var + eps) ** -1.5, dim=0)
        dmean = torch.sum(dx_hat * (-1) * (var + eps)**-0.5, dim=0) + dvar * torch.mean((-2) * (x - mean), 
                                                                                                  dim=0)
        dx = dx_hat * 1.0 / (var + eps) ** 0.5 + dvar * 2.0 / N * (x - mean) + dmean * 1.0 / N
        dgamma = torch.sum(dout * (x - mean) / (var + eps)**0.5, dim=0)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        out, cache = BatchNorm.forward(x.transpose(0, 1).flatten(start_dim=1).T, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dx, dgamma, dbeta = BatchNorm.backward(dout.transpose(0, 1).flatten(start_dim=1).T, cache)

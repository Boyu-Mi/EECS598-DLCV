...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            ...
            cls_conv = nn.Conv2d(in_channel, out_channel, 3, 1, 1)
            nn.init.normal_(cls_conv.weight.data, mean=0, std=0.01)
            nn.init.constant_(cls_conv.bias.data, val=0)
            stem_cls.append(cls_conv)
            stem_cls.append(nn.ReLU())
            
            box_conv = nn.Conv2d(in_channel, out_channel, 3, 1, 1)
            nn.init.normal_(box_conv.weight.data, mean=0, std=0.01)
            nn.init.constant_(box_conv.bias.data, val=0)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, feature in feats_per_fpn_level.items():
            class_logits[level] = self.pred_cls(self.stem_cls(feature)).flatten(start_dim=2).permute(0, 2, 1)
            boxes = self.stem_box(feature)
            boxreg_deltas[level] = self.pred_box(boxes).flatten(start_dim=2).permute(0, 2, 1)
            centerness_logits[level] = self.pred_ctr(boxes).flatten(start_dim=2).permute(0, 2, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = None
    mask = (gt_boxes == -1)[:, :4]
    cols = torch.chunk(gt_boxes, chunks=gt_boxes.shape[1], dim=1)
    x1, y1, x2, y2 = cols[0], cols[1], cols[2], cols[3]
    xc, yc = torch.chunk(locations, 2, dim=1)
    deltas = torch.hstack(
...
def fcos_apply_deltas_to_locations(
    ...
    mask = deltas < 0
    deltas = deltas * (~mask)
    output_boxes = torch.cat((locations, locations), dim=1) + torch.cat((-deltas[:, :2], deltas[:, 2:4]), dim=1) * stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    mask = (deltas == -1)[:, 0]
    cols = deltas.chunk(4, dim=1)
    l, t, r, b = cols[0], cols[1], cols[2], cols[3]
    centerness = torch.sqrt(
        torch.minimum(l, r) * torch.minimum(t, b) / (torch.maximum(l, r) * torch.maximum(t, b))
    ).squeeze()
    centerness[mask] = -1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        res_lst = self.pred_net.forward(fpn_feats)
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = res_lst[0], res_lst[1], res_lst[2]
        ######################################################################
        ...
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats_shape, self.backbone.fpn_strides, device=images.device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        gt_cls = matched_gt_boxes[:, :, -1].to(dtype=torch.int64) + 1# (batch_size, num_locations_across_fpn_levels, 1)
        gt_class = None #(batch_size, num_locations_across_fpn_levels, num_classes)
        #1 -> num_classes: -1 -> torch.zeros(num_classes); i >= 0 -> a = torch.zeros(num_classes) ; a[i] = 1
        trans_mat = torch.eye(self.num_classes + 1, device=gt_cls.device)[:, 1:]
        gt_class = trans_mat[gt_cls]
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_boxes = fcos_apply_deltas_to_locations(
                level_deltas[idx].reshape(-1, 4), 
                level_locations[idx].reshape(-1, 2), 
                stride)

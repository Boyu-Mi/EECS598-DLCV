...
class RPNPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for key_ in feats_per_fpn_level.keys():
            t = self.stem_rpn(feats_per_fpn_level[key_])
            obj_logits = self.pred_obj(t)
            box_logits = self.pred_box(t)
    ...
    def forward(
        ...
        for i in range(gt_boxes.shape[0]):
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes, gt_boxes[i],
                                                             iou_thresholds=self.anchor_iou_thresholds).to(anchor_boxes.device))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            # the first len(fg_idx) elements in anchor_box is foreground.
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        in_features = roi_size[0] * roi_size[1] * stem_channels[-1]
